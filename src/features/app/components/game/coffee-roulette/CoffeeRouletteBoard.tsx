...
import { DEFAULT_COFFEE_CHAT_DURATION_MINUTES, DEFAULT_MAX_PROMPTS, TIMER_WARNING_SECONDS } from '@/features/app/constants/coffeeRoulette';
import type { GameParticipantRecord, GamesSocketLike, EventsSocketLike } from '@/features/app/types/socket';
...
export interface CoffeeRouletteBoardProps extends BaseGameBoardProps {
  ...
  eventId?: string;
  onEmitAction: (actionType: string, payload?: Record<string, unknown>) => Promise<void>;
...
export function CoffeeRouletteBoard({
  ...
  const chatDurationMinutes = Math.max(1, Number(snapshot?.chatDurationMinutes || DEFAULT_COFFEE_CHAT_DURATION_MINUTES));
