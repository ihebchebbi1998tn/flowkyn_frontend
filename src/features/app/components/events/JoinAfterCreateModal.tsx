...
export function JoinAfterCreateModal({ isOpen, onClose, onJoin, eventId, gameId }: JoinAfterCreateModalProps) {
  const { t } = useTranslation();
  const [copied, setCopied] = useState(false);

  const joinLink = eventId
    ? `${window.location.origin}/join/${eventId}${gameId ? `?game=${gameId}` : ''}`
    : '';
