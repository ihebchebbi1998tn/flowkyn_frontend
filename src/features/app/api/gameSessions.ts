...
export const gameSessionsApi = {
  ...
  getActiveSessionsForEvent: (eventId: string) => {
    ...
    return api.get<ActiveSession[]>(`/events/${eventId}/game-sessions/active-list`).then(
      (result) => {
        // Ensure we always return an array even if backend returns a single object
        const sessions = Array.isArray(result) ? result : (result ? [result] : []);
        console.log(`[gameSessionsApi] ✅ getActiveSessionsForEvent success:`, { count: sessions.length });
        return sessions;
      },
      (error) => {
