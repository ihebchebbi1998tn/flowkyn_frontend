...
import { ArrowLeft, Clock, Copy, CheckCircle2, Gamepad2, Link as LinkIcon, Share2, Users, WifiOff } from 'lucide-react';
...
export function FocusedLayout() {
  ...
  const gameId = searchParams.get('activity') || '1';
  ...
  const joinLink = useMemo(() => {
    ...
    return `${window.location.origin}/join/${header.eventId}?activity=${gameId}`;
  ...
  // Timer based on server sessionStartedAt — survives page refreshes.
  const sessionStartIso = header?.sessionStartedAt ?? null;
  useEffect(() => {
    if (!sessionStartIso) { setElapsed(0); return; }
    const startMs = new Date(sessionStartIso).getTime();
    if (!Number.isFinite(startMs)) { setElapsed(0); return; }
    ...
    <GameHeaderContext.Provider value={{ header, setHeader }}>
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <header className="sticky top-0 z-40 border-b border-border/50 bg-background backdrop-blur-xl">
          <div className="flex items-center justify-end px-4 sm:px-6 h-14 max-w-[1600px] mx-auto w-full bg-primary-foreground">
            {!showGameHeader ?
          ...
          <div className="border-t border-border px-4 sm:px-6 py-3 backdrop-blur-sm bg-primary-foreground">
              <div className="relative max-w-[1600px] mx-auto">
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-2 text-[11px] text-muted-foreground shrink-0">
                    <LinkIcon className="h-3.5 w-3.5" />
                    <span className="hidden sm:inline font-medium">{t('gamePlay.shell.joinLink', 'Join link')}</span>
                  </div>
                  <div className="relative flex-1 min-w-0">
                    <Input
                      value={joinLink}
                      readOnly
                      className="h-8 text-[11px] sm:text-[12px] bg-background/50 border-border/30 rounded-lg pr-2 font-mono tracking-tight text-muted-foreground focus-visible:ring-primary/20"
                    />
                  </div>
                  <Button
                    size="sm"
                    ...
                    onClick={copyLink}
                    className={`h-8 text-[11px] gap-1.5 shrink-0 rounded-lg transition-all duration-300 ${
                      copied
                        ? 'bg-emerald-500/90 hover:bg-emerald-500 text-white border-emerald-500/50 shadow-sm shadow-emerald-500/20'
