...
function GamePlayWithoutBoundary() {
  ...
  const participants: GameParticipant[] = rawParticipants
    ...
    .map((p) => {
      ...
      const av = typeof rec.avatarUrl === 'string' ? rec.avatarUrl
        ...
        : null;
      const name = typeof rec.name === 'string' && rec.name ? rec.name : t('common.unknown', { defaultValue: 'Unknown' });
      return {
  ...
  // ─── Game session ──────────────────────────────────────────────────────────
  const {
    sessionId, setSessionId, activeRoundId, setActiveRoundId,
  ...
  useEffect(() => {
    ...
    eventPublicObj?.event_mode, eventPublicObj?.title, eventPublicObj?.description, disconnectedBadgeCount, t]);
